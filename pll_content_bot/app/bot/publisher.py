...
class TelegramPublisher:
    def __init__(self, bot: Bot, settings: Settings, repo: ContentRepository, runtime_repo: RuntimeSettingsRepository):
        self.bot = bot
    ...
    def is_auto_publish(self) -> bool:
        return self.runtime_repo.get_bool("auto_publish", default=self.settings.auto_publish)

    async def publish_or_moderate(self, rubric_type: str, source_item_id: str, text: str) -> int:
        auto_publish = self.is_auto_publish()
        status = ContentStatus.PUBLISHED if auto_publish else ContentStatus.DRAFT
